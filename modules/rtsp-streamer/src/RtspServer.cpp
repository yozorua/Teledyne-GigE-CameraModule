...
// Extract the URL path component from a full RTSP URL.
// "rtsp://host:port/cam0" → "/cam0"
// Also strips common RTSP track-ID suffixes.
static std::string ExtractPath(const std::string& url) {
    ...
    // Strip track-ID suffixes that some clients append (e.g. /trackID=0).
    for (const char* suffix : {"/streamid=0", "/trackID=0", "/track1", "/track0"}) {
        size_t suf = path.rfind(suffix);
        if (suf != std::string::npos) {
            path = path.substr(0, suf);
            break;
        }
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// RtspServer — stream registration
// ─────────────────────────────────────────────────────────────────────────────

void RtspServer::AddStream(int32_t camera_id, FrameGrabber& grabber, H264Encoder& encoder) {
    CameraStream cs;
    cs.camera_id = camera_id;
    cs.path      = "/cam" + std::to_string(camera_id);
    cs.grabber   = &grabber;
    cs.encoder   = &encoder;
    streams_.push_back(std::move(cs));
    rtp_timestamps_.push_back(0);
}
...
void RtspServer::Start(uint16_t port, int fps) {
    if (streams_.empty()) {
        std::cerr << "[RtspServer] No camera streams registered — cannot start.\n";
        return;
    }
    ...
    // One encode+send thread per camera stream.
    for (int32_t i = 0; i < (int32_t)streams_.size(); ++i)
        stream_threads_.emplace_back(&RtspServer::StreamLoop, this, i);

    accept_thread_ = std::thread(&RtspServer::AcceptLoop, this);

    std::cout << "[RtspServer] Listening on rtsp://0.0.0.0:" << port << "\n";
    for (const auto& cs : streams_)
        std::cout << "  Stream  rtsp://0.0.0.0:" << port << cs.path
                  << "  (camera " << cs.camera_id << ")\n";

    // Block until Stop() closes the listen socket.
    if (accept_thread_.joinable()) accept_thread_.join();
    for (auto& t : stream_threads_) if (t.joinable()) t.join();
...
int32_t RtspServer::FindStreamByPath(const std::string& path) const {
    // Exact match first.
    for (int32_t i = 0; i < (int32_t)streams_.size(); ++i)
        if (streams_[i].path == path) return i;

    // Prefix match: /cam0/trackID=0 should match /cam0.
    for (int32_t i = 0; i < (int32_t)streams_.size(); ++i) {
        const auto& p = streams_[i].path;
        if (path.size() > p.size() && path.substr(0, p.size()) == p)
            return i;
    }

    // Generic paths → first stream.
    if (path == "/" || path == "/stream" || path.empty())
        return streams_.empty() ? -1 : 0;

    return -1;
}

// ─────────────────────────────────────────────────────────────────────────────
// AcceptLoop — spawns a ClientLoop thread per connection
// ─────────────────────────────────────────────────────────────────────────────
...
void RtspServer::AcceptLoop() {
    while (running_.load(std::memory_order_acquire)) {
        sockaddr_in ca{};
        int len = sizeof(ca);
        SOCKET client = accept(listen_sock_, (sockaddr*)&ca, &len);
...
std::string RtspServer::BuildSdp(int32_t stream_idx) {
    H264Encoder* enc = streams_[stream_idx].encoder;
    std::string profile = enc->ProfileLevelId().empty() ? "42001f" : enc->ProfileLevelId();
    std::string sprop   = enc->SpropParameterSets();

    std::string sdp;
    sdp += "v=0\r\n";
    sdp += "o=- 0 0 IN IP4 0.0.0.0\r\n";
    sdp += "s=GigE Camera " + std::to_string(streams_[stream_idx].camera_id) + "\r\n";
...
// ─────────────────────────────────────────────────────────────────────────────
// ClientLoop — RTSP signaling for one client connection
// ─────────────────────────────────────────────────────────────────────────────
...
void RtspServer::ClientLoop(SOCKET sock, std::string client_addr) {
    ...
    int32_t     stream_idx  = -1;  // locked in on DESCRIBE

    while (running_.load(std::memory_order_acquire)) {
        RtspRequest req;
        if (!ReadRequest(sock, req)) break;

        std::cout << "[RtspServer] " << req.method << " " << req.path
                  << " from " << client_addr << "\n";
        ...
        if (req.method == "OPTIONS") {
            ...
            stream_idx = FindStreamByPath(req.path);
            if (stream_idx < 0) {
                std::string resp = "RTSP/1.0 404 Not Found\r\nCSeq: " +
                                   std::to_string(req.cseq) + "\r\n\r\n";
                send(sock, resp.c_str(), (int)resp.size(), 0);
                continue;
            }
            std::string sdp  = BuildSdp(stream_idx);
            ...
            // Resolve stream from SETUP URL; fall back to DESCRIBE's result.
            int32_t setup_idx = FindStreamByPath(req.path);
            if (setup_idx < 0) setup_idx = stream_idx;
            if (setup_idx < 0) setup_idx = 0;
...
// ─────────────────────────────────────────────────────────────────────────────
// StreamLoop — grab → encode → RTP send (one thread per camera)
// ─────────────────────────────────────────────────────────────────────────────
...
void RtspServer::StreamLoop(int32_t stream_idx) {
    ...
    while (running_.load(std::memory_order_acquire)) {
        ...
        // Skip if no playing clients are watching this stream.
        {
            std::lock_guard<std::mutex> lk(sessions_mutex_);
            bool any = false;
            for (const auto& s : sessions_)
                if (s.playing && s.stream_idx == stream_idx) { any = true; break; }
            if (!any) continue;
        }
...
void RtspServer::SendFrame(int32_t stream_idx, const std::vector<NaluUnit>& nalus, uint32_t rtp_ts) {
    std::lock_guard<std::mutex> lk(sessions_mutex_);
    for (auto& session : sessions_) {
        if (!session.playing || session.stream_idx != stream_idx) continue;
        for (size_t i = 0; i < nalus.size(); ++i)
            PacketizeNalu(session, nalus[i], rtp_ts, i == nalus.size() - 1);
    }
}
...
void RtspServer::PacketizeNalu(RtspSession& s, const NaluUnit& nalu,
    ...
    if (size <= MAX_RTP_PAYLOAD) {
        ...
        while (offset < size) {
            ...
            SendRtpPacket(s, is_last && last_in_frame, rtp_ts, payload.data(), payload.size());
            offset += chunk;
        }
    }
}
